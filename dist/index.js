...
// src/action/index.ts
var actionsCore = __toESM(require_core());
...
async function readManifest(packagePath) {
  ...
  } else if (import_node_path.default.extname(manifestPath) === "") {
    manifestPath = import_node_path.default.join(manifestPath, "package.json");
  }
  let json;
  try {
    json = await import_promises.default.readFile(manifestPath, "utf8");
  } catch (error) {
    throw (0, import_ono.ono)(error, `Unable to read ${manifestPath}`);
  }
  try {
    const { name, version: version2 } = JSON.parse(json);
    if (typeof name !== "string" || name.trim().length === 0) {
      throw new TypeError("Invalid package name");
    }
    if (typeof version2 !== "string" || version2.trim().length === 0) {
      throw new TypeError("Invalid package version");
    }
    return { name: name.trim(), version: version2.trim() };
  } catch (error) {
    throw (0, import_ono.ono)(error, `Unable to parse ${manifestPath}`);
  }
}
...
var DEFAULT_REGISTRY_URL = new URL("https://registry.npmjs.org/");
var DEFAULT_TAG = "latest";
var DEFAULT_ACCESS = "public";
var DEFAULT_ACCESS_SCOPED = "restricted";
var DEFAULT_STRATEGY = "upgrade";
var setValue = (value, defaultValue) => ({
  value: value ?? defaultValue,
  isDefault: value === void 0 || value === defaultValue
});
var setUrlValue = (value, defaultValue) => ({
  value: value ?? defaultValue,
  isDefault: value === void 0 || (value == null ? void 0 : value.href) === defaultValue.href
});
function normalizeOptions(options, manifest) {
  let defaultRegistryUrl = DEFAULT_REGISTRY_URL;
  let defaultTag = DEFAULT_TAG;
  let defaultAccess = manifest.scope ? DEFAULT_ACCESS_SCOPED : DEFAULT_ACCESS;
  let registryUrl;
  if (options.registryUrl !== void 0) {
    try {
      registryUrl = new URL(options.registryUrl);
    } catch {
      throw new InvalidRegistryUrlError(options.registryUrl);
    }
  }
  if (options.token !== void 0 && typeof options.token !== "string") {
  ...
  const publishConfig = {
    packageSpec: setValue(void 0, ""),
    tag: setValue(void 0, defaultTag),
    access: setValue(void 0, defaultAccess),
    dryRun: setValue(void 0, false),
    strategy: setValue(void 0, DEFAULT_STRATEGY)
  };
...
async function useNpmEnv(auth, task) {
  ...
  const tempDir = await import_promises2.default.mkdtemp(import_node_path2.default.join(import_node_os.default.tmpdir(), "npm-publish-"));
  ...
  const userConfig = await import_promises2.default.readFile(userConfigFile, "utf8").catch((error) => {
  ...
  const env = { npm_config_userconfig: tempConfigFile };
  if (token.value) {
    env.NODE_AUTH_TOKEN = token.value;
  }
  await import_promises2.default.writeFile(tempConfigFile, config, "utf8");
  try {
    return await task(env);
  } finally {
    await import_promises2.default.rm(tempDir, { force: true, recursive: true });
  }
}
...
function compareVersions(versions, publishConfig) {
  throw new Error("ah");
}
...
async function main() {
  ...
  try {
    ...
    const options = {
      token: getInput2("token", true),
      package: getInput2("package"),
      registryUrl: getInput2("registryUrl"),
      tag: getInput2("tag"),
      access: getInput2("access"),
      strategy: getInput2("strategy"),
      dryRun: ((_a = getInput2("dry-run")) == null ? void 0 : _a.toLowerCase()) === "true"
    };
    let results = await npmPublish(options);
    if (!results.releaseType) {
      console.log(
        `
\u{1F4E6} ${results.name} v${results.version} is already published to ${results.registryUrl}`
      );
    }
    if (results.releaseType === "lower") {
      console.log(
        `
\u{1F4E6} ${results.name} v${results.version} is lower than the version published to ${results.registryUrl}`
      );
    } else if (results.dryRun) {
      console.log(
        `
\u{1F4E6} ${results.name} v${results.version} was NOT actually published to ${results.registryUrl} (dry run)`
      );
    } else {
      console.log(
        `
\u{1F4E6} Successfully published ${results.name} v${results.version} to ${results.registryUrl}`
      );
    }
    setOutput2("id", results.id);
    setOutput2("name", results.name);
    setOutput2("version", results.version);
    setOutput2("release-type", results.releaseType);
    setOutput2("previous-version", results.previousVersion);
    setOutput2("tag", results.tag);
    setOutput2("access", results.access);
    setOutput2("registry-url", results.registryUrl);
    setOutput2("dry-run", results.dryRun);
  } catch (error) {
    errorHandler(error);
  }
}
