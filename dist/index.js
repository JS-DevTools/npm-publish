...
var isManifest = (name) => {
  return typeof name === "string" && import_node_path.default.basename(name) === MANIFEST_BASENAME;
};
var isDirectory = (name) => {
  return typeof name === "string" && import_node_path.default.extname(name) === "";
};
...
async function readManifest(packagePath) {
  ...
  if (!packagePath) {
    ...
    packageSpec = import_node_path.default.dirname(packagePath);
    manifestPath = packagePath;
  } else if (isDirectory(packagePath)) {
    packageSpec = packagePath;
    manifestPath = import_node_path.default.join(packagePath, MANIFEST_BASENAME);
  } else {
  ...
  let publishConfig;
  try {
    manifestContents = await import_promises.default.readFile(manifestPath, "utf8");
  } catch (error) {
    throw new PackageJsonReadError(manifestPath, error);
  }
  try {
    ...
    throw new PackageJsonParseError(manifestPath, error);
  }
...
var execProcess = (command, environment = {}) => {
...
async function publish(packageSpec, publishConfig, authConfig) {
...
function compareVersions(version2, publishedVersions, publishConfig) {
  ...
  const previousVersion = (0, import_semver2.valid)(tags[publishTag.value]) ?? void 0;
  const isUnique = !existingVersions.includes(version2);
  let releaseType;
  if (isUnique) {
    if (!previousVersion) {
      releaseType = INITIAL;
    } else if ((0, import_semver2.gt)(version2, previousVersion)) {
      releaseType = (0, import_semver2.diff)(version2, previousVersion) ?? void 0;
...
async function npmPublish(options) {
  ...
  let publishResult;
  if (versionComparison.releaseType !== void 0) {
...
async function run() {
  ...
  setOutput("type", results.releaseType, "");
  setOutput("old-version", results.previousVersion, "");
